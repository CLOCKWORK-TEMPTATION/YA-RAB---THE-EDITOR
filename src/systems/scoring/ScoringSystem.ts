// src/systems/scoring/ScoringSystem.ts
// =====================================
// Advanced Scoring System
//
// Extracted from THEditor.tsx - Scoring System (1:1 migration)
//
// Responsibilities:
// - Calculate classification confidence scores
// - Score lines as character, dialogue, action, parenthetical, scene-header
// - Calculate doubt scores
// - Extract top 2 candidates
// - Apply smart fallback
// - Classify with scoring (main entry point)
//
// NO classification logic (that's in engine/)
...
// These functions should be imported from the classifier module
// For now, they're stubs to allow compilation
// They will be provided via context in actual usage
type ClassifierHelperFunctions = {
  isActionVerbStart: (normalized: string) => boolean;
  matchesActionStartPattern: (normalized: string) => boolean;
  isSceneHeaderStart: (normalized: string) => boolean;
  isTransition: (line: string) => boolean;
  isCharacterLine: (line: string) => boolean;
  isLikelyAction: (line: string) => boolean;
};

// Default helpers that return false (safe defaults)
const defaultHelpers: ClassifierHelperFunctions = {
  isActionVerbStart: () => false,
  matchesActionStartPattern: () => false,
  isSceneHeaderStart: () => false,
  isTransition: () => false,
  isCharacterLine: () => false,
  isLikelyAction: () => false,
};

// Context helpers that can be set from outside
let _helpers: ClassifierHelperFunctions = defaultHelpers;

/**
 * Set the classifier helper functions
 * This should be called during initialization
 */
export function setClassifierHelpers(helpers: Partial<ClassifierHelperFunctions>): void {
  _helpers = { ...defaultHelpers, ...helpers };
}

function getHelpers(): ClassifierHelperFunctions {
  return _helpers;
}

function normalizeForAnalysisInternal(input: string): string {
...
export function buildContext(
  ...
  const WINDOW_SIZE = 3;
  const normalized = normalizeForAnalysisInternal(line);
...
export function getPrevNonBlankType(
  previousTypes: (string | null)[],
  currentIndex: number,
): string | null {
  for (let i = currentIndex - 1; i >= 0; i--) {
...
export function scoreAsCharacter(
  ...
  const looksLikeAction =
    helpers.isActionVerbStart(normalized) || helpers.matchesActionStartPattern(normalized);
  ...
  if (nextLine && !helpers.isSceneHeaderStart(nextLine) && !helpers.isTransition(nextLine)) {
  ...
  if (helpers.isActionVerbStart(normalized) || helpers.matchesActionStartPattern(normalized)) {
    score -= 20;
...
export function scoreAsDialogue(
  ...
  if (helpers.isSceneHeaderStart(normalized)) {
    score -= 20;
  ...
  const nextLine = ctx.nextLines[0]?.text;
  if (nextLine && !helpers.isCharacterLine(nextLine)) {
    score += 10;
...
export function scoreAsAction(
  ...
  if (helpers.isActionVerbStart(normalized)) {
  ...
  if (helpers.matchesActionStartPattern(normalized)) {
    score += 40;
  ...
  const nextLine = ctx.nextLines[0]?.text;
  if (nextLine && helpers.isLikelyAction(nextLine)) {
    score += 10;
  ...
  if (startsWithDash(rawLine) && !dialogueBlockInfo?.isInDialogueBlock) {
    ...
    if (helpers.isActionVerbStart(withoutDash)) {
      score += 30;
  ...
  if (helpers.isCharacterLine(normalized)) {
    score -= 20;
...
export function scoreAsSceneHeader(line: string, ctx: LineContext): ClassificationScore {
  ...
  if (nextLine && hasKnownPlace && nextLine.trim().length > 0) {
    if (!helpers.isCharacterLine(nextLine) && !helpers.isTransition(nextLine)) {
      score += 10;
...
function quickClassify(line: string, options?: QuickClassifyOptions): ClassificationResult | null {
  ...
  if (helpers.isSceneHeaderStart(normalizeLine(trimmed))) {
    return {
  ...
  if (helpers.isTransition(trimmed)) {
    return {
      type: "transition",
      confidence: "high",
      scores: {
